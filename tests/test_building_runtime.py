...
def test_cooling_humidification_energy_events_and_checkpoint() -> None:
    ...
    assert any(
        event.event_type == BuildingEventType.SENSOR_UPDATED
        for event in world.events
    )

    assert air.set_humidifier("k1324_humidifier_01", True, 3)["status"] == (
        "accepted"
    )
    ...
    assert restored.relative_humidity_pct == pytest.approx(
        cooled.relative_humidity_pct
    )
...
def test_k1324_system_time_advance_drives_runtime_and_sensor_app() -> None:
    ...
    assert hvac.set_hvac("k1324_hvac_01", True, "cooling", 22.0, 3)[
        "status"
    ] == "accepted"
