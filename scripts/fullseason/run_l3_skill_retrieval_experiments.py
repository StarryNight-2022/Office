...
RETRIEVED_CONTEXT_ENV_VAR = "FARM_FAIRY_RETRIEVED_CONTEXT_PATH"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cost-cap-dollars", type=float, default=9999.0)
    ...
    parser.add_argument("--parallel-tool-calls", type=_str_to_bool, default=True)
...
def build_runner_command(
    ...
    *,
    ...
    cmd = [
        ...
        "--parallel-tool-calls",
        "true" if args.parallel_tool_calls else "false",
...
def _load_candidate_agent_workflow(candidate_root: Path, scenario_id: str) -> dict[str, Any]:
    ...
    run_report_path = matches[0] / f"{scenario_id}.run_report.json"
    if run_report_path.is_file():
        run_report = json.loads(run_report_path.read_text(encoding="utf-8"))
        workflow_raw = (run_report.get("artifacts") or {}).get("agent_workflow")
        if workflow_raw:
            workflow_path = Path(workflow_raw)
            if not workflow_path.is_absolute():
                workflow_path = _resolve_candidate_path(candidate_root, workflow_raw)
            if not workflow_path.is_file():
                raise FileNotFoundError(
                    f"agent_workflow path does not exist: {workflow_path}"
                )
    ...
    if not output_jsonl.is_file():
        raise FileNotFoundError(
            f"No FAIRY run_report or legacy output.jsonl for {scenario_id}: {matches[0]}"
        )
...
def _oracle_workflow(scenario_id: str) -> dict[str, Any]:
    from fairy.scenarios.workflow_validation import workflow_from_oracle_events

    scenario_cls = _load_registry().get_scenario(scenario_id)
    scenario = scenario_cls()
    scenario.setup()
    workflow = workflow_from_oracle_events(scenario)
    current_time = float(getattr(scenario, "start_time", 0.0) or 0.0)
    for step in workflow.values():
        step["time"] = current_time
        if step.get("tool_name") == "SystemApp__advance_time":
            current_time += _advance_time_seconds(step.get("tool_args") or {})
    return workflow


def _metric_score(oracle_wf: dict[str, Any], agent_wf: dict[str, Any]) -> float:
    from fairy.scenarios.fos.spatiotemporal import compute_farm_fos
...
def _l2_scenarios_by_source() -> dict[str, list[str]]:
    base = (
        REPO_ROOT
        / "fairy/scenarios/farm_world_fullseason_v2/l2_l1_splits"
    )
...
def _l2_farming_groups_by_source() -> dict[tuple[str, str], str]:
    ...
    base = (
        REPO_ROOT
        / "fairy/scenarios/farm_world_fullseason_v2/l2_l1_splits/knowledge_library_pilot"
    )
