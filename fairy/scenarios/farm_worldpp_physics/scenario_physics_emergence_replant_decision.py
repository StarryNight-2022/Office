...
class ScenarioPhysicsEmergenceReplantDecision(Scenario):
    ...
    def _configure_initial_state(self) -> None:
        ...
        for i in range(64):
            ...
            r.growth_stage = (
                "VE" if not (_BAD_START <= i <= _BAD_END) else "PLANTED_PRE_EMERGENCE"
            )
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            o_survey = (
                mavic.fly_survey(_BAD_START - 2, _BAD_END + 2)
                .oracle()
                .with_id("o_survey_low_stand_block")
            ...
            o_robot = (
                robot.inspect_emergence(_BAD_START, _BAD_END)
                .oracle()
                .with_id("o_ground_check_emergence")
            ...
            o_tractor = (
                ...
                .depends_on(o_robot, delay_seconds=1)
            )
            o_inventory = (
            ...
            o_replant_a = (
                tractor.replant_seeds(
                    _BAD_START, _BAD_START + 3, depth_cm=4.0, spacing_cm=5.0
                )
                .oracle()
                .with_id("o_replant_failed_block_a")
                .depends_on(o_load, delay_seconds=2)
            )
            o_replant_b = (
                tractor.replant_seeds(
                    _BAD_START + 4, _BAD_END, depth_cm=4.0, spacing_cm=5.0
                )
                .oracle()
                .with_id("o_replant_failed_block_b")
                .depends_on(o_replant_a, delay_seconds=2)
            )
            o_commit = (
                farm_world.commit_daily_physics()
                .oracle()
                .with_id("o_commit_replant_effect")
                .depends_on(o_replant_b, delay_seconds=1)
            )
            o_report = (
                aui.send_message_to_user(
                    content="已确认12-19垄出苗失败，并只对失败区域补种。"
                )
                .oracle()
                .with_id("o_report")
