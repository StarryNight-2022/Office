...
SCENARIO_ID = "scenario_l1_hb_two_dry_patches_harvest_store_ready"
CHECKPOINT_DIR = Path(__file__).resolve().parent / "checkpoints"
CHECKPOINT_LABEL = "o_wait_harvest_day_066"


@register_scenario(SCENARIO_ID)
class ScenarioL1HBTwoDryPatchesHarvestStoreReady(Scenario):
    ...
    def build_events_flow(self) -> None:
        briefing = (
            "全田已到可收窗口。请最后复核天气、预报、土壤通行性、grain_moisture和仓储/烘干资源，"
            "然后按4垄一趟收获0-63垄，中途卸粮，按实际水分先烘干到安全目标后入库。"
        ...
        self.events = build_harvest_l1_events(
            ...
            report_text="已完成两旱斑场景的全田收获和安全入库。",
            start_ridge=0,
            end_ridge=63,
    ...
    def validate(self, env):
        return validate_native_workflow(self, env, "two dry patches harvest L1 split")
