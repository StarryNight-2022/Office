...
SCENARIO_ID = 'scenario_l2_hb_hn58_dual_stress_establishment_planting'


@register_scenario(SCENARIO_ID)
class ScenarioL2HbHn58DualStressEstablishmentPlanting(Scenario):
    ...
    def build_events_flow(self) -> None:
        if self.detailed_briefing:
            briefing_text = '任务：在大垄密植、一垄两行模式下，从源L3的真实开局状态完成HEINONG58建植窗口。请检查天气、预报、土壤、库存和拖拉机，完成整地、基肥、起垄后，按可见种植计划播种并复查建植结果。'
        else:
            briefing_text = '任务：完成HEINONG58大豆田建植；先检查天气、土壤、库存和设备，再整地、施基肥、起垄、播种并复查。'
        build_establishment_l2_flow(self, SPEC, briefing_text)

    def validate(self, env) -> ScenarioValidationResult:
        return validate_native_workflow(self, env, 'hb_heinong58_water_chemical_priority_under_dual_stress establishment planting L2')
