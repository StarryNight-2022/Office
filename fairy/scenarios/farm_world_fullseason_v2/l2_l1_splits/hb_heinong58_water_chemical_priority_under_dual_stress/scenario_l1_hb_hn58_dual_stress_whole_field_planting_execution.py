...
SCENARIO_ID = 'scenario_l1_hb_hn58_dual_stress_whole_field_planting_execution'


@register_scenario(SCENARIO_ID)
class ScenarioL1HbHn58DualStressWholeFieldPlantingExecution(Scenario):
    ...
    def build_events_flow(self) -> None:
        if self.detailed_briefing:
            briefing_text = '任务：承接已完成整地、基肥和起垄后的HEINONG58播种窗口。请复核当前天气、预报、土壤和播种机状态，按可见计划播种0-63垄并提交物理更新。'
        else:
            briefing_text = '任务：复核天气、土壤和设备后，按可见计划完成HEINONG58播种并复查。'
        build_planting_l1_flow(self, SPEC, 'whole_field', briefing_text)

    def validate(self, env) -> ScenarioValidationResult:
        return validate_native_workflow(self, env, 'hb_heinong58_water_chemical_priority_under_dual_stress planting L1')
