...
SCENARIO_ID = 'scenario_l2_hb_skip_rows_emergence_replant_28_31'


@register_scenario(SCENARIO_ID)
class ScenarioL2HbSkipRowsEmergenceReplant2831(Scenario):
    ...
    def build_events_flow(self) -> None:
        if self.detailed_briefing:
            briefing_text = '任务：从EMERGENCE窗口巡查后的真实状态出发，围绕stand-gap replanting做短闭环。请先用天气、预报、土壤、冠层、目标区状态、无人机和地面检查建立证据链；若证据和窗口支持，按当前资源约束对确认区域执行处理并复查响应。'
        else:
            briefing_text = '任务：诊断EMERGENCE窗口的stand-gap replanting需求；证据支持时处理确认区域并复查。'
        build_management_l2_flow(self, SPEC, SPEC.actions[1], 'o_emergence_replant_28_31', briefing_text)

    def validate(self, env) -> ScenarioValidationResult:
        return validate_native_workflow(self, env, 'hb_planter_skip_rows_stand_gap emergence_replant_28_31 L2')
