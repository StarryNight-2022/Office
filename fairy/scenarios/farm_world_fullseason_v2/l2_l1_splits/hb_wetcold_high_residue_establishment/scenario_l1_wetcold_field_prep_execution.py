...
SCENARIO_ID = "scenario_l1_hb_wetcold_high_residue_field_prep_execution"


@register_scenario(SCENARIO_ID)
class ScenarioL1HBWetcoldHighResidueFieldPrepExecution(Scenario):
    """L1 split: execute field prep/base fertilizer/ridge forming."""
    ...
    def build_events_flow(self) -> None:
        ...
        if self.detailed_briefing:
            briefing_text = (
                "本L1从湿冷高残茬田块未整地状态开始。"
                "先做最小复核：天气、种床土壤、库存和拖拉机状态；随后执行平地、基肥和起垄。"
                "不要播种。"
            )
        else:
            briefing_text = "请复核条件后执行整地、基肥和起垄；不要播种。"
        ...
        with EventRegisterer.capture_mode():
            ...
            o_report = aui.send_message_to_user(
                content="已完成湿冷高残茬田块耕整地动作包。"
            ).oracle().with_id("o_report").depends_on(o_detach_furrower, delay_seconds=1)
    ...
    def validate(self, env) -> ScenarioValidationResult:
        return validate_native_workflow(self, env, "wet-cold high-residue field-prep L1")
