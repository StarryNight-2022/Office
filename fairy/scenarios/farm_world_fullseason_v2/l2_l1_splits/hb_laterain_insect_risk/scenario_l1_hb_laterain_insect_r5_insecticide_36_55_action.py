...
CHECKPOINT_LABEL = 'before_o_r5_insecticide_0_36_55_action'
SPEC = get_spec(SPEC_SLUG)
SCENARIO_ID = 'scenario_l1_hb_laterain_insect_r5_insecticide_36_55_action'


@register_scenario(SCENARIO_ID)
class ScenarioL1HbLaterainInsectR5Insecticide3655Action(Scenario):
    ...
    def build_events_flow(self) -> None:
        if self.detailed_briefing:
            briefing_text = '任务：承接已确认的insect-control pesticide application action-ready 状态。请重新复核当前天气、短期预报、土壤/冠层、目标区状态、库存和设备；条件支持时完成一次处理并复查目标区。'
        else:
            briefing_text = '任务：复核条件后，对已确认区域执行一次insect-control pesticide application并复查。'
        build_management_l1_flow(self, SPEC, SPEC.actions[0], 'o_r5_insecticide_36_55', briefing_text)

    def validate(self, env) -> ScenarioValidationResult:
        return validate_native_workflow(self, env, 'hb_laterain_insect_risk r5_insecticide_36_55 L1')
