...
SOURCE_L3_PROFILE_NAME = "harbin_l3_wetjune_disease_recheck_after_fungicide_seed_1724"
CHECKPOINT_AFTER_MID_ROUTINE = "after_mid_routine_check"
CHECKPOINT_BEFORE_MID_FUNGICIDE = "before_mid_fungicide_action"
CHECKPOINT_AFTER_MID_FUNGICIDE = "after_o_mid_fungicide_0_20_43"
CHECKPOINT_AFTER_R5_ROUTINE = "after_r5_routine_check"
CHECKPOINT_AFTER_R5_FUNGICIDE = "after_o_r5_fungicide_0_20_43"
CHECKPOINT_HARVEST_R8_START = "o_wait_harvest_day_026"
CHECKPOINT_HARVEST_READY = "o_wait_harvest_day_035"
...
def populate_wetjune_recheck_apps(scenario) -> tuple[FarmWorldApp, WeatherApp]:
...
def restore_wetjune_recheck_checkpoint(scenario, checkpoint_label: str) -> dict[str, object]:
...
def apply_disease_block_fungicide_events(
    tractor: TractorApp,
    previous,
    prefix: str,
    *,
    liters_per_ridge: float = 3.6,
):
    current = previous
    events = []
...
def validate_native_workflow(
    ...
    if error_returns:
        return ScenarioValidationResult(
            ...
            rationale=(
                f"{rationale}\n"
                f"tool_error_returns={error_returns[:5]}"
            ),
        )
