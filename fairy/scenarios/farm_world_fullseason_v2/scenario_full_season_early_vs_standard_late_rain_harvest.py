...
class ScenarioFullSeasonEarlyVsStandardLateRainHarvest(Scenario):
    ...
    def init_and_populate_apps(self, *args: Any, **kwargs: Any) -> None:
        ...
        farm_world.configure_postharvest_market(
            name="commodity_late_rain_quality",
            market_moisture_pct=13.0,
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            o_commit_plant = (
                ...
                .depends_on(o_plant_b, delay_seconds=1)
            )
            o_after_plant = self._after_named_step(
                o_commit_plant, "after_ab_zone_planting"
            )
            ...
            o_a_emergence = (
                farm_world.get_ridge_range_state(A_START, A_END)
                .oracle()
                .with_id("o_a_emergence_range_state")
                .depends_on(o_emergence_overview, delay_seconds=1)
            )
            o_b_emergence = (
                farm_world.get_ridge_range_state(B_START, B_END)
                .oracle()
                .with_id("o_b_emergence_range_state")
                .depends_on(o_a_emergence, delay_seconds=1)
            )
            o_emergence_ndvi = (
                mavic.fly_survey(0, 63)
                .oracle()
                .with_id("o_emergence_ndvi_survey")
                .depends_on(o_b_emergence, delay_seconds=2)
            )
            o_robot_check = (
                robot.inspect_emergence(0, 15)
                .oracle()
                .with_id("o_emergence_ground_check")
                .depends_on(o_emergence_ndvi, delay_seconds=2)
            )

            o_wait_split = advance_days(
                self, o_robot_check, 45, "o_wait_ab_stage_split"
            )
            o_split_overview = (
                farm_world.get_farm_overview()
                .oracle()
                .with_id("o_ab_stage_split_farm_overview")
                .depends_on(o_wait_split, delay_seconds=1)
            )
            o_a_split = (
                farm_world.get_ridge_range_state(A_START, A_END)
                .oracle()
                .with_id("o_a_stage_split_range_state")
                .depends_on(o_split_overview, delay_seconds=1)
            )
            o_b_split = (
                farm_world.get_ridge_range_state(B_START, B_END)
                .oracle()
                .with_id("o_b_stage_split_range_state")
                .depends_on(o_a_split, delay_seconds=1)
            )
            o_split_soil = (
                sensor.read_soil_sensors()
                .oracle()
                .with_id("o_ab_stage_split_soil_check")
                .depends_on(o_b_split, delay_seconds=1)
            )
            o_split_ndvi = (
                mavic.fly_survey(0, 63)
                .oracle()
                .with_id("o_ab_stage_split_ndvi")
                .depends_on(o_split_soil, delay_seconds=2)
            )
            ...
            o_a_weather = (
                weather.get_current_weather()
                .oracle()
                .with_id("o_a_harvest_weather_check")
                .depends_on(o_wait_a_harvest, delay_seconds=1)
            )
            o_a_forecast = (
                weather.get_forecast(days=5)
                .oracle()
                .with_id("o_a_harvest_forecast_check")
                .depends_on(o_a_weather, delay_seconds=1)
            )
            o_a_soil = (
                sensor.read_soil_sensors()
                .oracle()
                .with_id("o_a_harvest_soil_check")
                .depends_on(o_a_forecast, delay_seconds=1)
            )
            o_a_overview = (
                farm_world.get_farm_overview()
                .oracle()
                .with_id("o_a_harvest_farm_overview")
                .depends_on(o_a_soil, delay_seconds=1)
            )
            o_a_range = (
                farm_world.get_ridge_range_state(A_START, A_END)
                .oracle()
                .with_id("o_a_harvest_range_state")
                .depends_on(o_a_overview, delay_seconds=1)
            )
            o_b_not_ready = (
                farm_world.get_ridge_range_state(B_START, B_END)
                .oracle()
                .with_id("o_b_same_day_not_ready_or_wetter")
                .depends_on(o_a_range, delay_seconds=1)
            )
            o_detach = (
                tractor.detach_implement()
                .oracle()
                .with_id("o_detach_before_a_harvest")
                .depends_on(o_b_not_ready, delay_seconds=1)
            )
            o_attach = (
                tractor.attach_implement("harvester")
                .oracle()
                .with_id("o_attach_harvester_for_a")
            ...
            o_after_a_harvest = self._after_named_step(
                o_harvest_a, "after_a_heike71_harvest_store"
            )
            ...
            o_b_weather = (
                weather.get_current_weather()
                .oracle()
                .with_id("o_b_harvest_weather_check")
                .depends_on(o_wait_b_harvest, delay_seconds=1)
            )
            o_b_forecast = (
                weather.get_forecast(days=5)
                .oracle()
                .with_id("o_b_harvest_forecast_check")
                .depends_on(o_b_weather, delay_seconds=1)
            )
            o_b_soil = (
                sensor.read_soil_sensors()
                .oracle()
                .with_id("o_b_harvest_soil_check")
                .depends_on(o_b_forecast, delay_seconds=1)
            )
            o_b_overview = (
                farm_world.get_farm_overview()
                .oracle()
                .with_id("o_b_harvest_farm_overview")
                .depends_on(o_b_soil, delay_seconds=1)
            )
            o_b_range = (
                farm_world.get_ridge_range_state(B_START, B_END)
                .oracle()
                .with_id("o_b_harvest_range_state")
                .depends_on(o_b_overview, delay_seconds=1)
            )
            ...
            o_after_b_harvest = self._after_named_step(
                o_harvest_b, "after_b_heinong84_harvest_store"
            )
            _ = (
                aui.send_message_to_user(
                    content="已完成A/B早熟与标准品种晚雨收获窗口场景：A区先收并干燥入库，B区按自己的成熟和水分窗口后收。"
                )
                .oracle()
                .with_id("o_report")
                .depends_on(o_after_b_harvest, delay_seconds=2)
            )
