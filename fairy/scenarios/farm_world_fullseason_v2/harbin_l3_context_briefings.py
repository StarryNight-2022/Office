...
TEXTSIM_TOP_K_ENV_VAR = "FARM_FAIRY_TEXTSIM_TOP_K"
LEGACY_KWOO_CONTEXT_ENV_VAR = "FARM_ARE_KWOO_CONTEXT_PATH"
LEGACY_RETRIEVED_CONTEXT_ENV_VAR = "FARM_ARE_RETRIEVED_CONTEXT_PATH"
LEGACY_TEXTSIM_TOP_K_ENV_VAR = "FARM_ARE_TEXTSIM_TOP_K"


def _get_env_with_legacy(name: str, legacy_name: str, default: str | None = None) -> str | None:
...
def _load_kwoo_context(scenario_id: str) -> str:
    path = _get_env_with_legacy(
        KWOO_CONTEXT_ENV_VAR,
        LEGACY_KWOO_CONTEXT_ENV_VAR,
        "scripts/kwoo_expansion_result.json",
    )
    if not path:
        raise ValueError(
...
def _textsim_differ_skill_context(spec: Any) -> str:
    ...
    top_k = int(
        _get_env_with_legacy(
            TEXTSIM_TOP_K_ENV_VAR,
            LEGACY_TEXTSIM_TOP_K_ENV_VAR,
            "6",
        )
        or "6"
    )
...
def _load_retrieved_context(scenario_id: str, mode: str) -> str:
    path = _get_env_with_legacy(
        RETRIEVED_CONTEXT_ENV_VAR,
        LEGACY_RETRIEVED_CONTEXT_ENV_VAR,
    )
    if not path:
        raise ValueError(
