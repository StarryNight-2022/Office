...
class ScenarioFullSeasonHeinong84ThresholdInsectLimitedSpray(Scenario):
    ...
    def init_and_populate_apps(self, *args: Any, **kwargs: Any) -> None:
        ...
        farm_world.configure_management_regime(
            regime="limited_insecticide",
            max_insecticide_applications=2,
            active_ingredient_cap_kg=0.18,
        )
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            o_emergence_robot = (
                ...
                .oracle()
                .with_id("o_emergence_ground_check")
                .depends_on(o_emergence_canopy, delay_seconds=2)
            )

            o_wait_early_signal = advance_days(
                self, o_emergence_robot, 38, "o_wait_early_insect_signal"
            )
            o_early_overview = (
                farm_world.get_farm_overview()
                .oracle()
                .with_id("o_early_insect_farm_overview")
                .depends_on(o_wait_early_signal, delay_seconds=1)
            )
            o_early_ndvi = (
                mavic.fly_survey(AFFECTED_START, AFFECTED_END)
                .oracle()
                .with_id("o_early_insect_ndvi_signal")
                .depends_on(o_early_overview, delay_seconds=2)
            )
            o_early_ground = (
                robot.inspect_crop_health(AFFECTED_START, AFFECTED_END)
                .oracle()
                .with_id("o_early_insect_ground_check_below_threshold")
                .depends_on(o_early_ndvi, delay_seconds=2)
            )
            o_no_spray_commit = (
                farm_world.commit_daily_physics()
                .oracle()
                .with_id("o_commit_no_spray_below_threshold")
                .depends_on(o_early_ground, delay_seconds=1)
            )
            o_after_no_spray = self._after_named_step(
                o_no_spray_commit, "after_below_threshold_insect_no_spray"
            )

            o_wait_threshold = advance_days(
                self, o_after_no_spray, 13, "o_wait_insect_threshold"
            )
            o_threshold_weather = (
                weather.get_current_weather()
                .oracle()
                .with_id("o_threshold_spray_weather_check")
                .depends_on(o_wait_threshold, delay_seconds=1)
            )
            o_threshold_forecast = (
                weather.get_forecast(days=3)
                .oracle()
                .with_id("o_threshold_spray_forecast_check")
                .depends_on(o_threshold_weather, delay_seconds=1)
            )
            o_threshold_soil = (
                sensor.read_soil_sensors()
                .oracle()
                .with_id("o_threshold_spray_soil_check")
                .depends_on(o_threshold_forecast, delay_seconds=1)
            )
            o_threshold_ndvi = (
                mavic.fly_survey(0, 63)
                .oracle()
                .with_id("o_threshold_insect_whole_field_ndvi")
                .depends_on(o_threshold_soil, delay_seconds=2)
            )
            o_threshold_ground = (
                robot.inspect_crop_health(AFFECTED_START, AFFECTED_END)
                .oracle()
                .with_id("o_threshold_insect_ground_confirmed")
                .depends_on(o_threshold_ndvi, delay_seconds=2)
            )
            o_load_insecticide = (
                tractor.load_pesticide(180.0)
                .oracle()
                .with_id("o_load_limited_insecticide")
            ...
            o_commit_spray = (
                farm_world.commit_daily_physics()
                .oracle()
                .with_id("o_commit_targeted_insecticide")
                .depends_on(o_spray, delay_seconds=1)
            )
            o_after_spray = self._after_named_step(
                o_commit_spray, "after_targeted_insecticide"
            )
            o_charge_before_recheck = (
                robot.charge()
                .oracle()
                .with_id("o_charge_robot_before_insect_recheck")
                .depends_on(o_after_spray, delay_seconds=1)
            )
            o_wait_robot_charge = (
                system.advance_time(hours=1)
                .oracle()
                .with_id("o_wait_robot_charge_before_insect_recheck")
                .depends_on(o_charge_before_recheck, delay_seconds=1)
            )
            o_recheck = (
                robot.inspect_crop_health(AFFECTED_START, AFFECTED_END)
                .oracle()
                .with_id("o_recheck_after_targeted_insecticide")
                .depends_on(o_wait_robot_charge, delay_seconds=2)
            )
            o_charge = (
                robot.charge()
                .oracle()
                .with_id("o_charge_robot_after_insect_recheck")
                .depends_on(o_recheck, delay_seconds=1)
            )

            o_wait_harvest = advance_days(self, o_charge, 48, "o_wait_harvest_window")
            o_harvest_weather = (
                weather.get_current_weather()
                .oracle()
            ...
            o_detach = (
                ...
                .oracle()
                .with_id("o_detach_before_harvest")
            ...
            _ = (
                aui.send_message_to_user(
                    content="已完成黑农84虫害阈值管理场景：早期未喷，后期只对18-37垄靶向喷药，并完成收获入库。"
                )
                .oracle()
                .with_id("o_report")
