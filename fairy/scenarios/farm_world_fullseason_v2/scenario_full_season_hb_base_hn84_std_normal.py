    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            o_robot_status_emergence = (
                ...
                .depends_on(o_wait_mavic_after_emergence, delay_seconds=1)
            )
            o_emergence_ground = (
            ...
            o_charge_after_emergence = (
                robot.charge()
                .oracle()
                .with_id("o_charge_robot_after_emergence")
                .depends_on(o_emergence_ground, delay_seconds=1)
            )

            o_wait_early = advance_days(
                self, o_charge_after_emergence, 23, "o_wait_early_growth"
            )
            o_early_soil = (
                sensor.read_soil_sensors()
                .oracle()
            ...
            o_early_ndvi = (
                mavic.fly_survey(0, 63)
                .oracle()
                .with_id("o_early_whole_field_ndvi")
                .depends_on(o_early_canopy, delay_seconds=2)
            )
            o_charge_mavic_after_early = (
                mavic.charge()
                .oracle()
                .with_id("o_charge_mavic_after_early_ndvi")
                .depends_on(o_early_ndvi, delay_seconds=1)
            )
            o_wait_mavic_after_early = (
                system.advance_time(hours=1)
                .oracle()
                .with_id("o_wait_mavic_charge_after_early_ndvi")
                .depends_on(o_charge_mavic_after_early, delay_seconds=1)
            )
            o_robot_status_early = (
                robot.check_status()
                .oracle()
                .with_id("o_robot_status_before_early_health")
                .depends_on(o_wait_mavic_after_early, delay_seconds=1)
            )
            o_early_health = (
            ...
            o_wait_r1 = advance_days(
                self, o_early_health, 22, "o_wait_r1_nutrient_window"
            )
            o_r1_soil = (
                sensor.read_soil_sensors()
                .oracle()
            ...
            o_mid_ndvi = (
                ...
                .depends_on(o_mid_soil, delay_seconds=2)
            )
            o_charge_mavic_after_mid = (
                mavic.charge()
                .oracle()
                .with_id("o_charge_mavic_after_midseason_ndvi")
                .depends_on(o_mid_ndvi, delay_seconds=1)
            )
            o_wait_mavic_after_mid = (
                system.advance_time(hours=1)
                .oracle()
                .with_id("o_wait_mavic_charge_after_midseason_ndvi")
                .depends_on(o_charge_mavic_after_mid, delay_seconds=1)
            )
            o_mid_robot_status = (
                robot.check_status()
                .oracle()
                .with_id("o_robot_status_before_midseason_pest_check")
                .depends_on(o_wait_mavic_after_mid, delay_seconds=1)
            )
            o_mid_pests = (
            ...
            o_wait_r5 = advance_days(self, o_mid_pests, 8, "o_wait_r5_r6_window")
            o_r5_weather = (
                weather.get_current_weather()
                .oracle()
            ...
            o_r5_thermal_west = (
                matrice.fly_survey(0, 31)
                .oracle()
                .with_id("o_r5_r6_west_thermal_reference")
                .depends_on(o_r5_canopy, delay_seconds=2)
            )
            o_charge_matrice_between_r5 = (
                matrice.charge()
                .oracle()
                .with_id("o_charge_matrice_between_r5_thermal_checks")
                .depends_on(o_r5_thermal_west, delay_seconds=1)
            )
            o_wait_matrice_between_r5 = (
                system.advance_time(hours=1)
                .oracle()
                .with_id("o_wait_matrice_charge_between_r5_thermal_checks")
                .depends_on(o_charge_matrice_between_r5, delay_seconds=1)
            )
            o_r5_thermal_east = (
                matrice.fly_survey(32, 63)
                .oracle()
                .with_id("o_r5_r6_east_thermal_reference")
                .depends_on(o_wait_matrice_between_r5, delay_seconds=2)
            )
            o_commit_r5_no_action = (
                farm_world.commit_daily_physics()
                .oracle()
                .with_id("o_commit_r5_r6_no_stress_action")
                .depends_on(o_r5_thermal_east, delay_seconds=1)
            )
            ...
            _ = (
                aui.send_message_to_user(
                    content="已完成哈尔滨黑农84标准密度正常年份baseline，并完成收获、干燥和入库。"
                )
                .oracle()
                .with_id("o_report")
