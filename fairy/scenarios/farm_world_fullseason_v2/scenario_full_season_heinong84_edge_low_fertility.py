...
class ScenarioFullSeasonHeinong84EdgeLowFertility(Scenario):
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            o_wait_edge_diag = self._advance_days(
                o_charge_robot_after_emergence, 7, "o_wait_edge_diagnosis_window"
            )
            o_diag_weather = (
                weather.get_current_weather()
                .oracle()
                .with_id("o_edge_diag_weather_check")
                .depends_on(o_wait_edge_diag, delay_seconds=1)
            )
            o_diag_soil = (
                sensor.read_soil_sensors()
                .oracle()
                .with_id("o_edge_diag_rule_out_water_stress")
                .depends_on(o_diag_weather, delay_seconds=1)
            )
            o_diag_canopy = (
                sensor.read_canopy_sensors()
                .oracle()
                .with_id("o_edge_diag_canopy_sensor_check")
                .depends_on(o_diag_soil, delay_seconds=1)
            )
            o_edge_ndvi = (
                mavic.fly_survey(0, 63)
                .oracle()
                .with_id("o_edge_whole_field_ndvi_survey")
                .depends_on(o_diag_canopy, delay_seconds=2)
            )
            o_reference_ndvi = (
                mavic.fly_survey(HEALTHY_REFERENCE_START, HEALTHY_REFERENCE_END)
                .oracle()
                .with_id("o_reference_ndvi_survey")
                .depends_on(o_edge_ndvi, delay_seconds=2)
            )
            o_edge_thermal = (
                matrice.fly_survey(0, 15)
                .oracle()
                .with_id("o_edge_thermal_rule_out_drought")
                .depends_on(o_reference_ndvi, delay_seconds=2)
            )
            o_robot_status_diag = (
                robot.check_status()
                .oracle()
                .with_id("o_robot_status_before_edge_ground_check")
                .depends_on(o_edge_thermal, delay_seconds=1)
            )
            o_edge_health = (
                robot.inspect_crop_health(EDGE_START, EDGE_END)
                .oracle()
                .with_id("o_edge_ground_leaf_color_stand_check")
                .depends_on(o_robot_status_diag, delay_seconds=2)
            )
            o_reference_health = (
                robot.inspect_crop_health(
                    HEALTHY_REFERENCE_START, HEALTHY_REFERENCE_END
                )
                .oracle()
                .with_id("o_reference_ground_health_check")
                .depends_on(o_edge_health, delay_seconds=2)
            )
            o_edge_pests = (
                robot.inspect_pests(EDGE_START, EDGE_END)
                .oracle()
                .with_id("o_edge_rule_out_pests")
                .depends_on(o_reference_health, delay_seconds=2)
            )
            o_edge_recovery_fertigation = (
                farm_world.apply_fertigation(
                    EDGE_START,
                    EDGE_END,
                    nutrient_amount=EDGE_RECOVERY_NUTRIENT_AMOUNT,
                    water_mm=2.0,
                )
                .oracle()
                .with_id("o_edge_targeted_nutrient_recovery")
                .depends_on(o_edge_pests, delay_seconds=2)
            )
            o_load_replant_seed = (
                tractor.load_seeds(SEED_TYPE, 60000)
                .oracle()
                .with_id("o_load_replant_seed")
                .depends_on(o_edge_recovery_fertigation, delay_seconds=1)
            )
            o_replant_edge = (
                tractor.replant_seeds(
                    SEVERE_EDGE_START,
                    SEVERE_EDGE_END,
                    4.0,
                    STANDARD_SPACING_CM,
                ...
                .oracle()
                .with_id("o_replant_severe_edge_0_3")
                .depends_on(o_load_replant_seed, delay_seconds=2)
            )
            o_commit_recovery = (
                farm_world.commit_daily_physics()
                .oracle()
                .with_id("o_commit_edge_recovery_management")
                .depends_on(o_replant_edge, delay_seconds=1)
            )
            o_after_recovery = self._after_named_step(
                o_commit_recovery, "after_edge_targeted_recovery"
            )

            o_wait_recovery = self._advance_days(
                o_after_recovery, 4, "o_wait_edge_recovery_recheck"
            )
            o_recheck_soil = (
                sensor.read_soil_sensors()
                .oracle()
                .with_id("o_edge_recovery_soil_recheck")
                .depends_on(o_wait_recovery, delay_seconds=1)
            )
            o_charge_mavic_before_recheck = (
                mavic.charge()
                .oracle()
                .with_id("o_charge_mavic_before_edge_recovery_ndvi_recheck")
                .depends_on(o_recheck_soil, delay_seconds=1)
            )
            o_wait_mavic_recheck_charge = (
                system.advance_time(hours=1)
                .oracle()
                .with_id("o_wait_mavic_edge_recovery_ndvi_recheck_charge")
                .depends_on(o_charge_mavic_before_recheck, delay_seconds=1)
            )
            o_recheck_ndvi = (
                mavic.fly_survey(0, 63)
                .oracle()
                .with_id("o_edge_recovery_whole_field_ndvi_recheck")
                .depends_on(o_wait_mavic_recheck_charge, delay_seconds=2)
            )
            o_charge_robot_before_recheck = (
                robot.charge()
                .oracle()
                .with_id("o_charge_robot_before_edge_recovery_recheck")
                .depends_on(o_recheck_ndvi, delay_seconds=1)
            )
            o_wait_robot_recheck_charge = (
                system.advance_time(hours=1)
                .oracle()
                .with_id("o_wait_robot_edge_recovery_charge")
                .depends_on(o_charge_robot_before_recheck, delay_seconds=1)
            )
            o_recheck_robot_status = (
                robot.check_status()
                .oracle()
                .with_id("o_robot_status_before_edge_recovery_recheck")
                .depends_on(o_wait_robot_recheck_charge, delay_seconds=1)
            )
            o_recheck_edge = (
                robot.inspect_crop_health(EDGE_START, EDGE_END)
                .oracle()
                .with_id("o_edge_recovery_ground_recheck")
                .depends_on(o_recheck_robot_status, delay_seconds=2)
            )
            o_charge_robot_after_recheck = (
                robot.charge()
                .oracle()
                .with_id("o_charge_robot_after_edge_recheck")
                .depends_on(o_recheck_edge, delay_seconds=1)
            )

            o_wait_r1 = self._advance_days(
                o_charge_robot_after_recheck, 18, "o_wait_r1_nutrient_window"
            )
            o_r1_soil = (
                sensor.read_soil_sensors()
                .oracle()
            ...
            o_r1_canopy = (
                sensor.read_canopy_sensors()
                .oracle()
                .with_id("o_r1_canopy_check")
                .depends_on(o_r1_soil, delay_seconds=1)
            )
            o_r1_edge = (
                robot.inspect_crop_health(EDGE_START, EDGE_END)
                .oracle()
                .with_id("o_r1_edge_nutrient_followup")
                .depends_on(o_r1_canopy, delay_seconds=2)
            )
            o_r1_reference = (
                robot.inspect_crop_health(
                    HEALTHY_REFERENCE_START, HEALTHY_REFERENCE_END
                ...
                .oracle()
                .with_id("o_r1_reference_followup")
                .depends_on(o_r1_edge, delay_seconds=2)
            )
            o_r1_edge_topdress = (
                farm_world.apply_fertigation(
                    EDGE_START,
                    EDGE_END,
                    nutrient_amount=R1_EDGE_NUTRIENT_AMOUNT,
                    water_mm=2.0,
                )
                .oracle()
                .with_id("o_r1_edge_light_nutrient_followup")
                .depends_on(o_r1_reference, delay_seconds=2)
            )
            o_commit_r1 = (
                farm_world.commit_daily_physics()
                .oracle()
                .with_id("o_commit_r1_edge_nutrient_management")
                .depends_on(o_r1_edge_topdress, delay_seconds=1)
            )
            o_after_r1 = self._after_named_step(
                o_commit_r1, "after_r1_edge_nutrient_followup"
            )
            ...
            o_mid_pests = (
                ...
                .oracle()
                .with_id("o_midseason_pest_and_disease_scout")
                .depends_on(o_mid_ndvi, delay_seconds=2)
            )
            ...
            o_wait_r5 = self._advance_days(
                o_charge_mavic_after_mid, 16, "o_wait_r5_r6_window"
            )
            o_pod_weather = (
                weather.get_current_weather()
                .oracle()
            ...
            o_r5_nutrient = (
                ...
                .oracle()
                .with_id("o_r5_small_whole_field_nutrient_support")
                .depends_on(o_pod_soil, delay_seconds=2)
            )
            o_commit_podfill = (
                farm_world.commit_daily_physics()
                .oracle()
                .with_id("o_commit_r5_r6_monitoring")
                .depends_on(o_r5_nutrient, delay_seconds=1)
            )
            o_after_podfill = self._after_named_step(
                o_commit_podfill, "after_r5_r6_monitoring"
            )
            ...
            o_commit_first_harvest = (
                farm_world.commit_daily_physics()
                .oracle()
                .with_id("o_commit_first_harvest_4_63")
                .depends_on(o_first_harvest, delay_seconds=1)
            )
            ...
            o_severe_harvest_weather = (
                weather.get_current_weather()
                .oracle()
                .with_id("o_severe_edge_harvest_weather_check")
                .depends_on(o_wait_severe_edge_maturity, delay_seconds=1)
            )
            o_severe_harvest_soil = (
                sensor.read_soil_sensors()
                .oracle()
                .with_id("o_severe_edge_harvest_trafficability_check")
                .depends_on(o_severe_harvest_weather, delay_seconds=1)
            )
            o_severe_harvest_range = (
                farm_world.get_ridge_range_state(SEVERE_EDGE_START, SEVERE_EDGE_END)
                .oracle()
                .with_id("o_severe_edge_range_maturity_moisture_check")
                .depends_on(o_severe_harvest_soil, delay_seconds=1)
            )
