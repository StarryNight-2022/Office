    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            o_wait_early = advance_days(
                self, o_emergence_ground, 24, "o_wait_early_growth"
            )
            o_early_soil = (
                sensor.read_soil_sensors()
                .oracle()
            ...
            o_wait_wet = advance_days(
                self, o_early_health, 18, "o_wait_wet_june_after_rain"
            )
            o_wet_weather = (
                weather.get_current_weather()
                .oracle()
            ...
            o_wet_soil = (
                sensor.read_soil_sensors()
                .oracle()
                .with_id("o_wet_june_soil_trafficability_check")
                .depends_on(o_wet_forecast, delay_seconds=1)
            )
            o_wait_spray_window = advance_days(
                self, o_wet_soil, 4, "o_wait_for_trafficable_post_rain_spray_window"
            )
            o_spray_weather = (
                weather.get_current_weather()
                .oracle()
                .with_id("o_post_rain_spray_weather_check")
                .depends_on(o_wait_spray_window, delay_seconds=1)
            )
            o_spray_soil = (
                sensor.read_soil_sensors()
                .oracle()
                .with_id("o_post_rain_spray_trafficability_soil_check")
                .depends_on(o_spray_weather, delay_seconds=1)
            )
            o_spray_canopy = (
                sensor.read_canopy_sensors()
                .oracle()
                .with_id("o_post_rain_sensor_flagged_canopy_check")
                .depends_on(o_spray_soil, delay_seconds=1)
            )
            o_affected_ndvi = (
                mavic.fly_survey(AFFECTED_START, AFFECTED_END)
                .oracle()
                .with_id("o_sensor_flagged_poordrainage_ndvi_detail")
                .depends_on(o_spray_canopy, delay_seconds=2)
            )
            o_charge_mavic_before_reference = (
                mavic.charge()
                .oracle()
                .with_id("o_charge_mavic_before_reference_drainage_ndvi")
                .depends_on(o_affected_ndvi, delay_seconds=1)
            )
            o_wait_mavic_before_reference = (
                system.advance_time(hours=1)
                .oracle()
                .with_id("o_wait_mavic_charge_before_reference_drainage_ndvi")
                .depends_on(o_charge_mavic_before_reference, delay_seconds=1)
            )
            o_reference_ndvi = (
                mavic.fly_survey(REFERENCE_START, REFERENCE_END)
                .oracle()
                .with_id("o_reference_normal_drainage_ndvi")
                .depends_on(o_wait_mavic_before_reference, delay_seconds=2)
            )
            o_affected_thermal = (
                matrice.fly_survey(AFFECTED_START, AFFECTED_END)
                .oracle()
                .with_id("o_sensor_flagged_poordrainage_thermal")
                .depends_on(o_reference_ndvi, delay_seconds=2)
            )
            o_robot_status_disease = (
                robot.check_status()
                .oracle()
                .with_id("o_robot_status_before_poordrainage_disease_check")
                .depends_on(o_affected_thermal, delay_seconds=1)
            )
            o_reference_ground = (
                robot.inspect_crop_health(REFERENCE_START, REFERENCE_END)
                .oracle()
                .with_id("o_reference_normal_drainage_ground_check")
                .depends_on(o_robot_status_disease, delay_seconds=2)
            )
            o_charge_robot_before_affected = (
                robot.charge()
                .oracle()
                .with_id("o_charge_robot_before_poordrainage_ground_confirm")
                .depends_on(o_reference_ground, delay_seconds=1)
            )
            o_wait_robot_before_affected = (
                system.advance_time(hours=1)
                .oracle()
                .with_id("o_wait_robot_charge_before_poordrainage_ground_confirm")
                .depends_on(o_charge_robot_before_affected, delay_seconds=1)
            )
            o_robot_status_affected = (
                robot.check_status()
                .oracle()
                .with_id("o_robot_status_before_poordrainage_ground_confirm")
                .depends_on(o_wait_robot_before_affected, delay_seconds=1)
            )
            o_affected_ground = (
                robot.inspect_crop_health(AFFECTED_START, AFFECTED_END)
                .oracle()
                .with_id("o_poordrainage_ground_confirm_disease")
                .depends_on(o_robot_status_affected, delay_seconds=2)
            )
            o_final_spray_weather = (
                weather.get_current_weather()
                .oracle()
                .with_id("o_final_targeted_fungicide_weather_check")
                .depends_on(o_affected_ground, delay_seconds=1)
            )
            o_final_spray_soil = (
                sensor.read_soil_sensors()
                .oracle()
                .with_id("o_final_targeted_fungicide_trafficability_check")
                .depends_on(o_final_spray_weather, delay_seconds=1)
            )
            o_load_fungicide = (
                tractor.load_fungicide(60.0)
                .oracle()
                .with_id("o_load_targeted_fungicide")
                .depends_on(o_final_spray_soil, delay_seconds=1)
            )
            o_fungicide = (
                tractor.apply_fungicide(
                    AFFECTED_START, AFFECTED_END, liters_per_ridge=FUNGICIDE_L_PER_RIDGE
                )
                .oracle()
                .with_id("o_apply_fungicide_poordrainage_44_53")
                .depends_on(o_load_fungicide, delay_seconds=2)
            )
            o_commit_disease = (
                farm_world.commit_daily_physics()
                .oracle()
                .with_id("o_commit_targeted_poordrainage_disease_management")
            ...
            o_charge_robot_after_disease = (
                robot.charge()
                .oracle()
                .with_id("o_charge_robot_after_poordrainage_disease_check")
                .depends_on(o_commit_disease, delay_seconds=1)
            )
            o_wait_robot_after_disease = (
                system.advance_time(hours=1)
                .oracle()
                .with_id("o_wait_robot_charge_after_poordrainage_disease_check")
                .depends_on(o_charge_robot_after_disease, delay_seconds=1)
            )

            o_wait_r1 = advance_days(
                self, o_wait_robot_after_disease, 10, "o_wait_r1_nutrient_window"
            )
            o_r1_soil = (
                sensor.read_soil_sensors()
                .oracle()
            ...
            _ = (
                aui.send_message_to_user(
                    content="已完成局部排水差湿六月病害场景：等待可作业窗口后仅对44-53垄靶向fungicide，并完成收获入库。"
                )
                .oracle()
                .with_id("o_report")
