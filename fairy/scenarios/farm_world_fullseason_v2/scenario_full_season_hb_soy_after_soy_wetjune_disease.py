...
class ScenarioFullSeasonHBSoyAfterSoyWetJuneDisease(Scenario):
    ...
    def init_and_populate_apps(self, *args: Any, **kwargs: Any) -> None:
        ...
        apply_prior_field_history(self, "soybean_after_soybean")
        apply_prior_field_history(
            self,
            "prior_disease_hotspot",
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            o_emergence_ndvi = (
                ...
                .oracle()
                .with_id("o_emergence_whole_field_ndvi_survey")
                .depends_on(o_emergence_canopy, delay_seconds=2)
            )
            ...
            o_wait_early = advance_days(
                self, o_emergence_ground, 20, "o_wait_early_growth"
            )
            o_early_soil = (
                sensor.read_soil_sensors()
                .oracle()
            ...
            o_wait_history_scout = advance_days(
                self, o_early_health, 15, "o_wait_early_wet_june_history_scout"
            )
            o_history_weather = (
                weather.get_current_weather()
                .oracle()
                .with_id("o_soy_history_wet_june_weather_check")
                .depends_on(o_wait_history_scout, delay_seconds=1)
            )
            o_history_forecast = (
                weather.get_forecast(days=4)
                .oracle()
                .with_id("o_soy_history_wet_june_forecast_check")
                .depends_on(o_history_weather, delay_seconds=1)
            )
            o_history_soil = (
                sensor.read_soil_sensors()
                .oracle()
                .with_id("o_soy_history_wet_june_soil_check")
                .depends_on(o_history_forecast, delay_seconds=1)
            )
            o_history_canopy = (
                sensor.read_canopy_sensors()
                .oracle()
                .with_id("o_soy_history_sensor_priority_canopy_check")
                .depends_on(o_history_soil, delay_seconds=1)
            )
            o_charge_mavic_before_history_detail = (
                mavic.charge()
                .oracle()
                .with_id("o_charge_mavic_before_soy_history_ndvi_detail")
                .depends_on(o_history_canopy, delay_seconds=1)
            )
            o_wait_mavic_before_history_detail = (
                system.advance_time(hours=1)
                .oracle()
                .with_id("o_wait_mavic_charge_before_soy_history_ndvi_detail")
                .depends_on(o_charge_mavic_before_history_detail, delay_seconds=1)
            )
            o_affected_ndvi = (
                mavic.fly_survey(AFFECTED_START, AFFECTED_END)
                .oracle()
                .with_id("o_soy_history_sensor_flagged_ndvi_detail")
                .depends_on(o_wait_mavic_before_history_detail, delay_seconds=2)
            )
            o_reference_ndvi = (
                mavic.fly_survey(REFERENCE_START, REFERENCE_END)
                .oracle()
                .with_id("o_soy_history_reference_ndvi")
                .depends_on(o_affected_ndvi, delay_seconds=2)
            )
            o_affected_thermal = (
                matrice.fly_survey(AFFECTED_START, AFFECTED_END)
                .oracle()
                .with_id("o_soy_history_sensor_flagged_thermal")
                .depends_on(o_reference_ndvi, delay_seconds=2)
            )
            o_robot_status_disease = (
                robot.check_status()
                .oracle()
                .with_id("o_robot_status_before_soy_history_disease_check")
                .depends_on(o_affected_thermal, delay_seconds=1)
            )
            o_reference_ground = (
                robot.inspect_crop_health(REFERENCE_START, REFERENCE_END)
                .oracle()
                .with_id("o_soy_history_reference_ground_check")
                .depends_on(o_robot_status_disease, delay_seconds=2)
            )
            o_charge_robot_before_affected = (
                robot.charge()
                .oracle()
                .with_id("o_charge_robot_before_soy_history_ground_confirm")
                .depends_on(o_reference_ground, delay_seconds=1)
            )
            o_wait_robot_before_affected = (
                system.advance_time(hours=1)
                .oracle()
                .with_id("o_wait_robot_charge_before_soy_history_ground_confirm")
                .depends_on(o_charge_robot_before_affected, delay_seconds=1)
            )
            o_robot_status_affected = (
                robot.check_status()
                .oracle()
                .with_id("o_robot_status_before_soy_history_ground_confirm")
                .depends_on(o_wait_robot_before_affected, delay_seconds=1)
            )
            o_affected_ground = (
                robot.inspect_crop_health(AFFECTED_START, AFFECTED_END)
                .oracle()
                .with_id("o_soy_history_ground_confirm_disease")
                .depends_on(o_robot_status_affected, delay_seconds=2)
            )
            o_charge_robot_before_pest_ruleout = (
                robot.charge()
                .oracle()
                .with_id("o_charge_robot_before_soy_history_pest_ruleout")
                .depends_on(o_affected_ground, delay_seconds=1)
            )
            o_wait_robot_before_pest_ruleout = (
                system.advance_time(hours=1)
                .oracle()
                .with_id("o_wait_robot_charge_before_soy_history_pest_ruleout")
                .depends_on(o_charge_robot_before_pest_ruleout, delay_seconds=1)
            )
            o_robot_status_pest_ruleout = (
                robot.check_status()
                .oracle()
                .with_id("o_robot_status_before_soy_history_pest_ruleout")
                .depends_on(o_wait_robot_before_pest_ruleout, delay_seconds=1)
            )
            o_ruleout_pests = (
                robot.inspect_pests(AFFECTED_START, AFFECTED_END)
                .oracle()
                .with_id("o_soy_history_rule_out_insects")
                .depends_on(o_robot_status_pest_ruleout, delay_seconds=2)
            )
            o_wait_spray_window = advance_days(
                self, o_ruleout_pests, 3, "o_wait_for_soy_history_spray_window"
            )
            o_spray_weather = (
                weather.get_current_weather()
                .oracle()
                .with_id("o_targeted_fungicide_weather_check")
                .depends_on(o_wait_spray_window, delay_seconds=1)
            )
            o_spray_soil = (
                sensor.read_soil_sensors()
                .oracle()
                .with_id("o_targeted_fungicide_trafficability_soil_check")
                .depends_on(o_spray_weather, delay_seconds=1)
            )
            o_load_fungicide = (
                tractor.load_fungicide(90.0)
                .oracle()
                .with_id("o_load_targeted_fungicide")
                .depends_on(o_spray_soil, delay_seconds=1)
            )
            ...
            o_commit_spray = (
                farm_world.commit_daily_physics()
                .oracle()
                .with_id("o_commit_soy_history_targeted_fungicide")
                .depends_on(o_spray, delay_seconds=1)
            )

            o_charge_robot_after_spray = (
                robot.charge()
                .oracle()
                .with_id("o_charge_robot_after_soy_history_disease_check")
                .depends_on(o_commit_spray, delay_seconds=1)
            )
            o_wait_robot_after_spray = (
                system.advance_time(hours=1)
                .oracle()
                .with_id("o_wait_robot_charge_after_soy_history_disease_check")
                .depends_on(o_charge_robot_after_spray, delay_seconds=1)
            )

            o_wait_r1 = advance_days(
                self, o_wait_robot_after_spray, 10, "o_wait_r1_nutrient_window"
            )
            o_r1_soil = (
                sensor.read_soil_sensors()
                .oracle()
            ...
            o_r5_commit = (
                farm_world.commit_daily_physics()
                .oracle()
                .with_id("o_commit_r5_r6_no_extra_water_action")
                .depends_on(o_r5_canopy, delay_seconds=1)
            )
            ...
            _ = (
                aui.send_message_to_user(
                    content="已完成前茬大豆/病害历史湿六月场景：根据历史风险提前scout，并仅对22-43垄靶向fungicide后完成收获入库。"
                )
                .oracle()
                .with_id("o_report")
