...
from fairy.scenarios.fos.gates import GateResult, GateSpec
from fairy.scenarios.fos.metrics import (
    EfficiencyBreakdown,
    FOSComponents,
    FOSReport,
    OutcomeBreakdown,
...
def _count_safety_violations(env: Any) -> tuple[int, list[dict[str, Any]]]:
    ...
    """
    from fairy.types import EventType
...
def _oracle_tool_count(scenario: Any) -> int:
    ...
    FAIRY stores migrated oracle flows as CapturedEvent objects. Older ARE
    traces may still expose OracleEvent-like objects with make_event(); support
    both shapes so replay evaluation keeps the same denominator.
    """
    ...
    for event in getattr(scenario, "events", None) or []:
        ...
        if action is None and app is not None and function is not None:
...
def _try_get_farm_world(scenario: Any):
    ...
    if not callable(getter):
        return None
    try:
        from fairy.apps.farm_world import FarmWorldApp
