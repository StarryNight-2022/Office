...
@register_scenario("scenario_full_season_aphid_threshold_V2")
class ScenarioFullSeasonAphidThreshold(Scenario):
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            o_wait_pest = system.advance_time(days=45).oracle().with_id("o_wait_to_pest_window").depends_on(o_monitor_1, delay_seconds=1)
            o_pest_signal = mavic.fly_survey(14, 30).oracle().with_id("o_pest_window_weak_ndvi_signal").depends_on(o_wait_pest, delay_seconds=2)
            o_ground_pest0 = robot.inspect_pests(20, 22).oracle().with_id("o_ground_pest_below_threshold").depends_on(o_pest_signal, delay_seconds=2)
            ...
            o_wait_podfill = system.advance_time(days=35).oracle().with_id("o_wait_to_pod_fill_monitoring").depends_on(o_commit_pest, delay_seconds=1)
