...
from fairy.types import EventRegisterer, event_registered
...
class DailyStateTraceApp(App):
    """Scenario-local oracle probe for daily FARM engine state.

    This is deliberately defined in the v2 scenario file so the production
    FarmWorld app and the existing balanced scenario stay unchanged.
    """
    ...
    def capture_daily_state(
        ...
        """Return one daily snapshot of weather, field summary, and engine state."""
        ...
        payload: dict[str, Any] = {
            "label": label,
            "sim_datetime_utc": now.isoformat(),
            "weather": weather,
        ...
        top_vwcs: list[float] = []
        root_vwcs: list[float] = []
        lai_values: list[float] = []
        ndvi_values: list[float] = []
        nutrient_values: list[float] = []
        nutrient_stress_values: list[float] = []
        disease_values: list[float] = []
        insect_values: list[float] = []
        yield_potential_values: list[float] = []
        ...
        for rid in ridge_ids:
            ...
            top_vwcs.append(float(soil.top_vwc))
            root_vwcs.append(float(soil.root_vwc))
            lai_values.append(float(canopy.lai))
            ndvi_values.append(float(canopy.ndvi_proxy))
            nutrient_values.append(float(mgmt.nutrient_index))
            nutrient_stress_values.append(float(mgmt.nutrient_stress))
            disease_values.append(float(biotic.disease_pressure))
            insect_values.append(float(biotic.insect_pressure))
            yield_potential_values.append(float(canopy.yield_potential_g_m2))
                ...
                ridges.append(
                    {
                        ...
                        "root_vwc": round(float(soil.root_vwc), 4),
        ...
        payload.update(
            {
                ...
                "field_summary": {
                    ...
                    "avg_top_vwc": avg(top_vwcs),
                    "min_top_vwc": round(min(top_vwcs), 4),
                    "avg_root_vwc": avg(root_vwcs),
                    "min_root_vwc": round(min(root_vwcs), 4),
                    "avg_lai": avg(lai_values),
                    "max_lai": round(max(lai_values), 4),
                    "avg_ndvi_proxy": avg(ndvi_values),
                    "min_ndvi_proxy": round(min(ndvi_values), 4),
                    "avg_nutrient_index": avg(nutrient_values),
                    "min_nutrient_index": round(min(nutrient_values), 4),
                    "min_nutrient_stress": round(min(nutrient_stress_values), 4),
                    "max_disease_pressure": round(max(disease_values), 4),
                    "max_insect_pressure": round(max(insect_values), 4),
                    "avg_yield_potential_g_m2": avg(yield_potential_values),
                    "max_yield_potential_g_m2": round(
                        max(yield_potential_values),
                        4,
                    ),
                },
            }
...
@register_scenario("scenario_full_season_balanced_v2")
class ScenarioFullSeasonBalancedV2(Scenario):
    """
    ...
    def init_and_populate_apps(self, *args, **kwargs) -> None:
        ...
        daily_trace = DailyStateTraceApp(farm_world_app=farm_world, weather_app=weather)
        mavic = DroneApp(
    ...
    def _advance_days_with_daily_trace(
        self,
        prev: Any,
        days: int,
        prefix: str,
        *,
        ...
        """Advance one calendar day at a time and emit one engine trace per day."""
        system = self.get_typed_app(SystemApp)
        daily_trace = self.get_typed_app(DailyStateTraceApp, "DailyStateTrace")
        for day_index in range(1, days + 1):
            ...
            prev = (
                daily_trace.capture_daily_state(
                    f"{prefix}_day_{day_index:03d}",
                    include_ridge_details=include_ridge_details,
                )
                .oracle()
                .with_id(f"{prefix}_trace_day_{day_index:03d}")
                .depends_on(prev, delay_seconds=1)
            )
        return prev
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            ...
            o_trace_after_planting = daily_trace.capture_daily_state("after_planting_commit", True).oracle().with_id("o_trace_after_planting_commit").depends_on(o_commit_planting, delay_seconds=1)

            # 2) Emergence and early monitoring.
            o_wait_emergence = self._advance_days_with_daily_trace(o_trace_after_planting, 12, "o_wait_to_emergence_window")
            ...
            o_wait_nutrient = self._advance_days_with_daily_trace(o_monitor_1, 32, "o_wait_to_v4_nutrient_window")
            ...
            o_trace_after_nutrient = daily_trace.capture_daily_state("after_nutrient_management", True).oracle().with_id("o_trace_after_nutrient_management").depends_on(o_commit_nutrient, delay_seconds=1)


            # Pod-fill water management branch.
            o_wait_podfill = self._advance_days_with_daily_trace(o_trace_after_nutrient, 35, "o_wait_to_r5_r6_pod_fill")
            ...
            o_trace_after_podfill = daily_trace.capture_daily_state("after_pod_fill_management", True).oracle().with_id("o_trace_after_pod_fill_management").depends_on(o_commit_podfill, delay_seconds=1)

            # 5) Harvest branch: wait to maturity, assess moisture, dry-down if needed, harvest, dry/store.
            o_wait_maturity = self._advance_days_with_daily_trace(o_trace_after_podfill, 45, "o_wait_to_r8_maturity")
            ...
            o_drydown_wait = self._advance_days_with_daily_trace(o_harvest_overview, 1, "o_wait_drydown_if_needed")
            ...
            o_trace_after_harvest = daily_trace.capture_daily_state("after_harvest_commit", True).oracle().with_id("o_trace_after_harvest_commit").depends_on(o_commit_harvest, delay_seconds=1)
            o_inventory_post = farm_world.get_inventory().oracle().with_id("o_check_postharvest_inventory").depends_on(o_trace_after_harvest, delay_seconds=1)
