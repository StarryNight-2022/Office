...
from fairy.controllers.agent_config import ResearchAgentProfileConfig
...
class ResearchStrategyCoordinator:
    """FAIRY port of ARE's FARM research-strategy coordinator."""
    ...
    def parse_rewoo_plan(task: str, max_plan_steps: int) -> list[str]:
        ...
        for step in numbered_steps:
            ...
            if len(normalized_step) < 4 or normalized_step in cleaned_steps:
                continue
    ...
    def reset(self, telemetry: dict[str, object]) -> None:
        ...
        for key, value in {
            ...
            self.telemetry.setdefault(key, value)
    ...
    def _build_tree_search_context(self, task: str) -> str:
        ...
        if len(self._tree_candidates) == 0:
            return ""
        selected_index = 1 if self._tree_error_signal and len(self._tree_candidates) > 1 else 0
        if selected_index == 1:
    ...
    def _generate_tree_candidates(self, task: str) -> list[_TreeCandidate]:
        ...
        ) or ["Read current farm state", "Check weather and sensors", "Execute lowest-risk required action"]
        ...
        for branch_index in range(max(1, self.family_config.tree_search.branch_factor)):
            if branch_index == 0:
                ...
                branch_plan = " -> ".join(
                    ["Validate preconditions first", *base_steps[: self.family_config.tree_search.search_depth]]
                )
            else:
                branch_plan = " -> ".join(
                    ["Gather additional evidence via sensors", *base_steps[: self.family_config.tree_search.search_depth]]
                )
            candidates.append(_TreeCandidate(plan=branch_plan, score=self._score_branch(branch_plan)))
    ...
    def _build_critic_context(self, task: str) -> str:
        revision_cycles = max(1, min(max(1, self.family_config.critic.max_revision_cycles), 2))
        self.telemetry["critic_cycles"] = int(self.telemetry["critic_cycles"]) + 1
        self.telemetry["revision_cycles"] = int(self.telemetry["revision_cycles"]) + revision_cycles
        actor_steps = self.parse_rewoo_plan(task, max_plan_steps=3) or [
            "Inspect state",
    ...
    def _build_graph_memory_context(self, task: str) -> str:
        parsed_steps = self.parse_rewoo_plan(task, max_plan_steps=3) or [task[:180]]
    ...
    def _append_graph_node(self, kind: str, text: str) -> None:
        ...
        if len(self._graph_nodes) > 1:
            self._graph_edges.add((self._graph_nodes[-2][0], node_id))
    ...
    def _retrieve_graph_nodes(self, task: str) -> list[str]:
        ...
        for _, node_text in self._graph_nodes:
            overlap = len(tokens.intersection(set(re.findall(r"\w+", node_text.lower()))))
    ...
    def _detect_contradiction(self, node_texts: list[str]) -> bool:
        ...
        for positive_pattern, negative_pattern in [
            (r"(?<!no )\brain\b", r"\bno rain\b"),
            (r"\bdry\b", r"\bwet\b"),
            (r"\bsuccess\b", r"\bfailed\b"),
            (r"\bavailable\b", r"\bunavailable\b"),
        ]:
            if re.search(positive_pattern, merged) and re.search(negative_pattern, merged):
                return True
        return False
