...
class DynamicSkillLibrary:
    """Small local port of ARE's JSON skill retrieval library."""
    ...
    def load(self) -> None:
        ...
        if not self.skill_dir.exists() or not self.skill_dir.is_dir():
            logger.warning("Skill library directory not found: %s", self.skill_dir)
            return
        ...
        for skill_path in sorted(self.skill_dir.glob("*.json")):
            try:
                ...
                logger.warning("Failed to load skill file %s: %s", skill_path, exc)
